...
<template>
  ...
  <main>
    ...
    <textarea v-model="nodes" placeholder="Enter nodes here"></textarea>
    <button @click="findViablePairs">Find Viable Pairs</button>
    <p>Number of viable pairs: <span v-if="viablePairs !== 0">{{ viablePairs }}</span></p>
    <h2>Part two</h2>
    <p>
...
<script>
  ...
  export default {
    ...
    methods: {
        findViablePairs() {
            const nodes = this.nodes.split('\n').slice(2).map(line => {
                const parts = line.split(/\s+/);
                const [, x, y] = parts[0].match(/x(\d+)-y(\d+)/);
